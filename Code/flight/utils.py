...
def addPlaces():
    file = open("./Data/myAirports.csv", "r")
    print("Adding Airports...")
    total = get_number_of_lines("./Data/myAirports.csv")
...
def addDomesticFlights():
    file = open("./Data/my_domestic_flights.csv", "r")
    print("Adding Domestic Flights...")
    total = get_number_of_lines("./Data/my_domestic_flights.csv")
    for i, line in tqdm(enumerate(file), total=total):
        if i == 0:
            continue
        data = line.split(',')
